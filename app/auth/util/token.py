...
def token_required(f):
    ...
    def decorated(current_data,*args, **kwargs):
        token=None
        if 'sv-token' in request.headers:
            token =request.headers['sv-token']
        ...
        try:
            current_data=jwt.decode(token,SECRET_KEY,algorithm='HS256')
               ...
               return  {'status': "error","data": {"code":500,"message": "Token expired. Login to get new one."}}, 200
        except jwt.InvalidSignatureError:
               return  {'status': "error","data": {"code":500,"message": "Token’s signature doesn’t match . "}}, 200
        except jwt.DecodeError:
               return  {'status': "error","data": {"code":500,"message": "Token cannot be decoded because it failed validation."}}, 200
